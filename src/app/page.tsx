...
import {
  ...
  BarChart3, Activity, Trophy, CalendarDays, Users2, RefreshCcw, ChevronRight
} from "lucide-react";
  ...
  return (
    <main className="min-h-screen bg-[#fdfbf7] flex flex-col items-center relative overflow-x-hidden">
      ...
      <header className="w-full max-w-6xl flex justify-between items-center mt-8 mb-10 px-6">
        ...
        {/* 🚀 반응형 분양상담사 전용 버튼: PC에선 텍스트 버튼, 모바일에선 둥근 아이콘 */}
      ...
      <div className="w-full max-w-6xl px-4 md:px-6 text-center mb-8">
        ...
        <div className="relative w-full max-w-xl mx-auto mb-10 group mt-8">
          <input type="text" placeholder="어떤 지역, 어떤 아파트를 찾으세요?" className="w-full px-5 py-4 pr-16 rounded-[24px] border-none shadow-sm focus:ring-4 focus:ring-orange-100 text-[15px] font-bold outline-none bg-white transition-all" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          <button className="absolute right-3 top-3 bottom-3 w-12 bg-[#4A403A] text-white rounded-2xl flex items-center justify-center shadow-md hover:bg-black transition-all"><Search strokeWidth={3} size={22} /></button>
        </div>
